...
import { FiCalendar, FiMapPin, FiUsers, FiFilter, FiCreditCard } from 'react-icons/fi';
...
const Bookings = () => {
  ...
  const handlePay = async (id) => {
    setPaying(id);
    try {
      const res = await API.post(`/bookings/${id}/pay`, { paymentMethod: 'card' });
                  ...
                  <div className="booking-card-footer">
                    ...
                    <div className="table-actions">
                      ...
                      {b.paymentStatus === 'unpaid' && b.status !== 'cancelled' && (
                        <button className="btn btn-primary btn-sm" disabled={paying === b._id} onClick={() => handlePay(b._id)}>
                          <FiCreditCard /> {paying === b._id ? 'Paying...' : 'Pay Now'}
                        </button>
                      )}
            ...
            <div className="modal booking-detail-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-actions">
                ...
                {selected.paymentStatus === 'unpaid' && selected.status !== 'cancelled' && (
                  <button className="btn btn-primary" disabled={paying === selected._id} onClick={() => handlePay(selected._id)}>
                    <FiCreditCard /> Pay ${selected.totalAmount}
                  </button>
                )}
